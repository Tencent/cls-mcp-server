...
const SearchLogRequestSchema = {
  From: z
    .number()
    .describe(
      '要检索分析的日志的起始时间，Unix时间戳（需要传毫秒粒度）。To-From 时间范围建议不要过大，建议默认近15分钟，否则会导致返回的日志过多，影响性能。',
    ),
  To: z
    .number()
    .describe(
      '要检索分析的日志的结束时间，Unix时间戳（需要传毫秒粒度）。To-From 时间范围建议不要过大，建议默认近15分钟，否则会导致返回的日志过多，影响性能。',
    ),
